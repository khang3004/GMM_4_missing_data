...
\begin{frame}[shrink=7]{How the Repository Mirrors the Math}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.56\textwidth}
            \input{figures/tikz/repo_pipeline.tikz}
        \end{column}
...
\begin{frame}[shrink=4]{Code-to-Equation Mapping}
    ...
    \begin{tabular}{>{\bfseries}p{0.2\textwidth} p{0.33\textwidth} p{0.38\textwidth}}
        ...
        \toprule
        \(\scal{\gamma}_{jk}\) & posterior responsibilities & \texttt{\_e\_step()} with log-domain Gaussian scores \\
        \(\scal{\alpha}_k,\vect{\mu}_k,\mat{\Sigma}_k\) & standard GMM M-step & \texttt{\_m\_step\_params()} \\
        \(\vect{x}_{j,m}^{\star}\) & precision-weighted update for missing entries & \texttt{\_m\_step\_data()} \\
